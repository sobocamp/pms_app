@extends('layouts.template')

@section('title', $title)

@section('content')
<style>
    p {
        margin: 0;
    }
</style>
<a href="{{ route('pembelian.create') }}" class="btn btn-primary float-end mt-n1">
    <i class="fa fa-plus"></i> Tambah
</a>
<div class="mb-3">
    <h1 class="h3 d-inline align-middle">{{ $title }}</h1>
</div>

<div class="row">
    <div class="col-12 col-xl-12">
        <div class="card">
            <div class="table-responsive">
                <table class="table table-striped mb-0">
                    <thead>
                        <tr>
                            <th style="width: 10px;">No.</th>
                            <th>No. Pembelian</th>
                            <th>Tanggal</th>
                            <th>Supplier</th>
                            <th>Total</th>
                            <th width="60px;">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($pembelian as $item)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $item->purchase_number }}</td>
                            <td>{{ $item->purchase_date }}</td>
                            <td>{{ $item->supplier }}</td>
                            <td>{{ $item->total }}</td>
                            <td class="text-nowrap">
                                <a data-bs-toggle="tooltip" data-bs-placement="top" class="btn btn-sm btn-primary"
                                    href="{{ route('pembelian.show', $item->id) }}" title="Lihat">
                                    <i class="fa fa-eye"></i>
                                </a>

                                {{-- Edit --}}
                                <a data-bs-toggle="tooltip" data-bs-placement="top" class="btn btn-sm btn-warning"
                                    href="{{ route('pembelian.edit', $item->id) }}" title="Edit">
                                    <i class="fa fa-pencil"></i>
                                </a>

                                {{-- Delete --}}
                                <form action="{{ route('pembelian.destroy', $item->id) }}" method="post"
                                    class="d-inline">
                                    @method('delete')
                                    @csrf
                                    <button data-bs-toggle="tooltip" data-bs-placement="top" title="Hapus"
                                        class="btn btn-danger btn-sm"
                                        onClick="return confirm('Apakah anda yakin ingin menghapus data ini?')"><i
                                            class="fa fa-trash"></i></button>
                                </form>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            </div>
        </div>
        <div class="d-flex justify-content-end mt-3">
            {{ $pembelian->links() }}
        </div>
    </div>
</div>
@endsection
